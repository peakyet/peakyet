...
- Read `templates/README.md` and start from `beamer-deck.html`, the skill's only template: fixed 1280x720 frames, tinted head and foot bands, Beamer `block` boxes, overlays, and **the classic blue theme, `data-theme="beamer"`, which is the default for every deck**. `data-theme="paper"` is an opt-in variant (the repository's serif-on-paper palette); set it only when the user asks for it, never because a neighbouring page, an earlier deck, or the repository's long-form notes use that palette. Light background, dark text. Build a custom template only if the user asks for one.
